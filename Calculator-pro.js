    ...
    if (newNumber !== null) {
      if (isNaN(newNumber) || newNumber == "") {
        ...
        if (numberList.length === 1) {
          ...
          let sumas = ` El resultado de la suma es ${numberList
            .reduce(suma)
            .toFixed(3)
            .replace(/[.,]000$/, "")} \n`;
          let restas = `El resultado de la resta es ${numberList
            .reduce(resta)
            .toFixed(3)
            .replace(/[.,]000$/, "")} \n`;
          let div = `El resultado de la división es ${numberList
            .reduce(division)
            .toFixed(3)
            .replace(/[.,]000$/, "")} \n`;
          let multi = `El resultado de la multiplicación es ${numberList
