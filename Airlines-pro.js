...
function eliminar() {
  ...
  const posicionVuelo = flights.findIndex(
    (posicion) => posicion.id === eliminacion
  );
...
function mayor() {
  ...
  flights.forEach(function (mayorQue) {
    console.log(
      `El vuelo número ${mayorQue.id} con origen en ${mayorQue.from} con destino ${mayorQue.to} a ${mayorQue.cost} euros.`
    );
  });
}
...
function menor() {
  ...
  flights.forEach(function (menorQue) {
    console.log(
      `El vuelo número ${menorQue.id} con origen en ${menorQue.from} con destino ${menorQue.to} a ${menorQue.cost} euros.`
    );
  });
}
...
function admin() {
  let pregunta = prompt(
    `Escriba \"borrar\" para borrar vuelos o \"crear\" para crear vuelos`
  ).toLowerCase();
  if (pregunta === `borrar`) {
    eliminar();
...
function user() {
  let pregunta = parseFloat(
    prompt(
      `Para ordenar los vuelos: De mayor a menor pulse 1 \nDe menor a mayor pulse 2`
    )
  );
...
flights.forEach(function (vuelo) {
  ...
  console.log(
    `El vuelo con origen en ${vuelo.from} con destino ${vuelo.to} tiene un coste de ${vuelo.cost} y ${escala}.`
  );
});
...
console.log(
  `El coste medio de los vuelos es de ${(costeMedio / flights.length).toFixed(
    2
  )}`
);
...
console.log(
  `Hay ${flights.filter((vuelo) => vuelo.scale).length} vuelos con escala.`
);
...
for (i = flights.length - 1; i >= flights.length - numeroUltimosVuelos; i--) {
  console.log(
    `Vuelo con destino a ${flights[i].to} con origen en ${flights[i].from}`
  );
}
